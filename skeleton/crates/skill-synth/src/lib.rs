...
pub enum IdlType {
    U8, U16, U32, U64, I64, Bool, String,
...
pub struct IdlAccount {
    pub name: String,
    #[serde(default, alias = "isMut")] pub is_mut: bool,
    #[serde(default, alias = "isSigner")] pub is_signer: bool,
    #[serde(default)] pub pda: Option<IdlPda>,
}
...
pub struct IdlInstructionArg {
    pub name: String,
    #[serde(default, alias = "type", alias = "kind")] pub kind: String,
}

#[derive(Clone, Debug, Default, Serialize, Deserialize, PartialEq)]
pub struct IdlInstruction {
    pub name: String,
    #[serde(default)] pub args: Vec<IdlInstructionArg>,
    #[serde(default)] pub accounts: Vec<IdlAccount>,
    #[serde(default)] pub discriminator: Option<[u8; 8]>,
}
...
pub struct Idl {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")] pub address: Option<String>,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct SkillAccountSpec { pub name: String, pub is_mut: bool, pub is_signer: bool }

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct SkillArgSpec { pub name: String, pub ty: IdlType }
...
pub struct Skill {
    ...
    #[serde(default)] pub discriminator: [u8; 8],
    #[serde(default)] pub accounts: Vec<SkillAccountSpec>,
    #[serde(default)] pub args: Vec<SkillArgSpec>,
}
...
pub fn arg_schema(kind: &str) -> Value {
    match kind {
        "u8" | "u16" | "u32" | "u64" | "u128" | "i8" | "i16" | "i32" | "i64" => json!({ "type": "integer" }),
...
pub fn synthesize(program: &Program, idl: &Idl) -> Vec<Skill> {
    idl.instructions.iter().map(|ix| Skill {
        ...
        discriminator: ix.discriminator.unwrap_or_else(|| compute_anchor_discriminator(&ix.name)),
        accounts: ix.accounts.iter().map(|a| SkillAccountSpec {
            name: a.name.clone(), is_mut: a.is_mut, is_signer: a.is_signer,
        }).collect(),
        args: ix.args.iter().map(|a| SkillArgSpec {
            name: a.name.clone(), ty: parse_idl_type(&a.kind),
        }).collect(),
    }).collect()
}
...
mod tests {
    ...
    fn fixture() -> (Program, Idl) {
        ...
        let idl = Idl {
            ...
            instructions: vec![
                IdlInstruction { name: "initialize".into(), args: vec![IdlInstructionArg { name: "authority".into(), kind: "publicKey".into() }], ..Default::default() },
                IdlInstruction { name: "greet".into(), args: vec![IdlInstructionArg { name: "name".into(), kind: "string".into() }], ..Default::default() },
                IdlInstruction { name: "set_counter".into(), args: vec![IdlInstructionArg { name: "value".into(), kind: "u64".into() }], ..Default::default() },
            ],
            ..Default::default()
        };
        (program, idl)
    ...
    fn generated_schema_validates_valid_payload_and_rejects_invalid() {
        ...
        let counter_validator = JSONSchema::compile(&skills[2].params_schema).expect("valid schema");
    ...
    fn widened_idl_v030_roundtrip() {
        ...
        let program = Program { id: "HELLO111111111111111111111111111111111111111".into(), name: None };
    ...
    fn legacy_idl_without_discriminator_falls_back() {
        ...
        let program = Program { id: "LEGACY1111111111111111111111111111111111111".into(), name: None };
        let skill = &synthesize(&program, &idl)[0];
    ...
    fn borsh_types_deserialize_all_variants() {
        let cases = [
            ("u8", IdlType::U8), ("u16", IdlType::U16), ("u32", IdlType::U32),
            ("u64", IdlType::U64), ("i64", IdlType::I64), ("bool", IdlType::Bool),
            ("string", IdlType::String), ("pubkey", IdlType::Pubkey), ("bytes", IdlType::Bytes),
        ];
